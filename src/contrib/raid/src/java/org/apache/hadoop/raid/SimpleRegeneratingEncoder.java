...
public class SimpleRegeneratingEncoder extends Encoder {
  public static final Log LOG = LogFactory.getLog(
                                  "org.apache.hadoop.raid.SimpleRegeneratingencoder");
  private ErasureCode simpleRegeneratingCode;

  private int stripeSize;
  private int paritySize;
  private int srcParitySize;
  public SimpleRegeneratingEncoder(
    Configuration conf) {
    super(conf, Codec.getCodec("src"));
    this.simpleRegeneratingCode = new SimpleRegeneratingCode();
    this.simpleRegeneratingCode.init(codec);
    stripeSize = this.simpleRegeneratingCode.stripeSize();
    paritySize = this.simpleRegeneratingCode.paritySize();
    srcParitySize = ((SimpleRegeneratingCode)this.simpleRegeneratingCode).srcParitySize();
    LOG.info("MMM Initialed srcParitySize = "+srcParitySize);
  }
  ...
  void performEncode(byte[][] readBufs, byte[][] writeBufs, int idx,
    ...
    simpleRegeneratingCode.encode(data, code);
    for (int i = 0; i < paritySize; i++) {
