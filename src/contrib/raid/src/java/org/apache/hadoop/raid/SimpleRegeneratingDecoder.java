...
public class SimpleRegeneratingDecoder extends Decoder {
  public static final Log LOG = LogFactory.getLog(
                                  "org.apache.hadoop.raid.SimpleRegeneratingDecoder");
  ...
  public SimpleRegeneratingDecoder(
    Configuration conf) {
    super(conf, Codec.getCodec("src"));
    this.simpleRegeneratingCode = new SimpleRegeneratingCode[parallelism];
    stripeSize = this.codec.stripeLength;
    paritySize = this.codec.parityLength;
     
    for (int i = 0; i < parallelism; i++) {
      simpleRegeneratingCode[i] = new SimpleRegeneratingCode();
      simpleRegeneratingCode[i].init(codec);
    }
    srcParitySize = ((SimpleRegeneratingCode)simpleRegeneratingCode[0]).srcParitySize();
      ...
      try {
        ...
        for (int i = 0; i < parallelism; i++) {
          ...
          parallelDecoder.execute(new DecodeOp(
            ...
            erasedLocations, simpleRegeneratingCode[i]));
        }
